...
class SetOfProductViewSet(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk):
        set_of_product = Product.objects.with_favorite_flag(request.auth).with_all_rating().filter(
...
class ProductViewSet(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk):
        product = get_object_or_404(Product.objects.with_favorite_flag(request.auth).with_all_rating(), pk=pk)
...
class CategoryViewSet(viewsets.ViewSet):
    ...
    def retrieve(self, request, pk):
        category_products = Product.objects.with_favorite_flag(request).with_all_rating().filter(category_id=pk)
